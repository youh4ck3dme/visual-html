...
type PendingImport = { code: string; projectName: string };

export function BuilderWorkspace({ importProjectId }: { importProjectId?: string }) {
  const { t } = useT();
  ...
  const applyProjectImport = useCallback(
    (code: string, projectName: string) => {
      ...
      setVersions((p) => [
        ...p,
        makeVersion(code, "manual", t("builder.import.versionLabel", { name: projectName })),
      ]);
      setImportBanner(projectName);
      setImportWarning(null);
      addAi(t("builder.import.loaded", { name: projectName }));
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps -- makeVersion/id stable enough for import
    [t],
  );
  ...
  useEffect(() => {
    if (!hydrated || !importProjectId || importHandledRef.current) return;
    ...
    void (async () => {
      ...
      if (!project) {
        setImportWarning(t("builder.import.notFound"));
        void navigate({ to: "/builder", search: {}, replace: true });
        return;
      }

      const code = buildProjectExportHtml(project.result, project.name);
      const existingCode = readStored()?.generatedCode?.trim() ?? generatedCode.trim();
      if (existingCode) {
        setPendingImport({ code, projectName: project.name });
        setImportConfirmOpen(true);
      } else {
        applyProjectImport(code, project.name);
      }
      void navigate({ to: "/builder", search: {}, replace: true });
    })();
  }, [hydrated, importProjectId, generatedCode, applyProjectImport, navigate, t]);
  ...
  return (
    <div className="flex min-h-[calc(100dvh-3rem)] w-full flex-col overflow-hidden border border-shell-border bg-shell text-foreground">
      {importWarning && (
        <div
          className="flex items-center gap-2 border-b border-amber-500/40 bg-amber-500/10 px-4 py-2 text-sm text-amber-700 dark:text-amber-300"
          data-testid="builder-import-warning"
          role="status"
        >
      ...
      {importBanner && (
        <div
          className="flex items-center gap-2 border-b border-primary/30 bg-primary/5 px-4 py-2 text-sm text-foreground"
          data-testid="builder-import-banner"
          role="status"
        >
          <Sparkles className="h-4 w-4 shrink-0 text-primary" aria-hidden />
          {t("builder.import.banner", { name: importBanner })}
        </div>
      )}
          ...
          >
            ...
            <div className="relative">
              <Input
                ...
                placeholder={
                  isGenerating ? t("builder.inputWorking") : t("builder.inputPlaceholder")
                }
      ...
      <Dialog open={importConfirmOpen} onOpenChange={setImportConfirmOpen}>
        <DialogContent className="border-shell-border bg-shell-elevated sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("builder.import.confirmTitle")}</DialogTitle>
            <DialogDescription>{t("builder.import.confirmDescription")}</DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2 sm:gap-0">
            <Button
              ...
              data-testid="builder-import-cancel"
            >
              {t("builder.import.confirmCancel")}
            </Button>
            <Button
              type="button"
              onClick={() => {
                if (pendingImport) {
                  applyProjectImport(pendingImport.code, pendingImport.projectName);
                }
                setImportConfirmOpen(false);
                setPendingImport(null);
              }}
              data-testid="builder-import-confirm"
            >
              {t("builder.import.confirmReplace")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
