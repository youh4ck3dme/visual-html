...
src/pages/index-page.tsx       # thin re-export → <EditorModeScreenshot projectId={…} />
src/pages/projects-page.tsx    # thin re-export → <EditorModeProjects />
src/pages/project-detail-page.tsx # thin re-export → <EditorModeProjects initialProjectId={…} />
...
| `--skip-rate-limit` | Preskočí len rate-limit check (iPhone lane ho nastavuje automaticky) |
...
Auto-generated by TanStack Router during `npm run dev` or `npm run build`. **Never edit or commit manual changes.** If local drift appears (e.g. missing `/projects/$projectId`), run `npm run build` and commit only if the generator updated the file, or `git restore src/routeTree.gen.ts` to discard accidental edits.

### Deploy checklist

1. `npm run test:integrity:fast` — all checks green
2. `npm run test:integrity:iphone-17-air` — mobile/PWA lane
3. Merge PR to `main` — Vercel production deploy
4. Lighthouse on production HTTPS (not local `vite dev`)
